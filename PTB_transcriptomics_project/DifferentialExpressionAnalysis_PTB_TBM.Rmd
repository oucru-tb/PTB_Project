...
#E(network)$weight <- 2**((E(network)$weight - min(E(network)$weight)) / diff(range(E(network)$weight)))
V(network)$color <- rownames(mat)
...
l <- qgraph.layout.fruchtermanreingold(e,
                                       ...
                                       area=100,
                                       repulse.rad=(vcount(network)^4)
                                       )


library(comprehenr)
to_vec(for(i in 1:18) if(rownames(mat2)[i]%in%c('blue','brown','turquoise')) -85 )
...
plot(network,
     ...
     vertex.label.color = ifelse(rownames(mat2)%in%c('blue','brown'),  'white','black'),
     vertex.label.dist=ifelse(rownames(mat2)%in%c('blue','brown','turquoise'), 0, 1),
     vertex.label.degree = ifelse(rownames(mat2)%in%c('blue','brown','turquoise'), 0, 92),
       )
