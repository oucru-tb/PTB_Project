...
tmp <- signif(moduleTraitCor[[set]][module_order,c("log2_CT")], 2) %>% as.data.frame()
tmp<-tmp[match(rownames(mat), rownames(tmp)),]
tmp

colorRampPalette(c("blue", "red"))(length(tmp))
...
plot(network,
     ...
     edge.curved=0.08,
     vertex.frame.color= adjustcolor(colors$color[match(V(network)$degree, colors$levels)],0.9),
