...
df$CNIH4level<-factor(df$CNIH4level,levels=c("LOW","HIGH"))
df$LIMS_ID<-NULL
...
count_data <-count_data %>% filter(genenames %in% names(RNA_data))
...
res1 <- results(dds,name = 'CNIH4level_HIGH_vs_LOW')
res2 <- lfcShrink(dds, coef="CNIH4level_HIGH_vs_LOW", type="apeglm")
...
EnhancedVolcano(res1,
    ...
    y = 'pvalue',
    selectLab = c('TNFSF12','TSLP','TNFSF10','OSM','CSF1','LTA','IL33','IL27','IL18','IL17F','IL17A','IL15','IL13','CXCL8','IL7','IL6','IL4','IL2','CSF2','CSF3','FLT3LG','IL12A', 'IL12B', 'IL10', 'IL1B','IL6','IL4','IFNG','TNF'),
    )
```

```{r}
dim(All_sample_RNAseq)
describe(All_sample_RNAseq$Study)
All_sample_RNAseq<-All_sample_RNAseq %>%  mutate (Study=ifelse(Study=="HV","43TB",Study))
table(All_sample_RNAseq$timepoint)

All_sample_RNAseq_T0<-All_sample_RNAseq %>% subset(timepoint=="D0" & Study %in% c("19EI","26TB","27TB","28TB","29TB","43TB"))

table(All_sample_RNAseq_T0$Study)

All_sample_RNAseq_T0$Study<-factor(All_sample_RNAseq_T0$Study,levels=c("19EI","26TB","27TB","28TB","29TB","43TB"))

sample_names<-All_sample_RNAseq_T0$LIMS_ID

count_deseq2 <-count_all[,c(sample_names)]
dim(count_deseq2)

dds_r <- DESeqDataSetFromMatrix(count_deseq2[,1:100], colData=All_sample_RNAseq_T0[1:100,], ~Study)
## annotate dds and res
dds_anno <- anno_gene_symbol(data = dds_r,data_anno = anno,export = F,name = NULL)
