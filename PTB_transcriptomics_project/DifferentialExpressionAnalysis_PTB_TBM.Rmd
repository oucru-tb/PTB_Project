...
mat <- mat2
mat[p.mat2>=0.05] <- 0
mat[mat<0.5&mat>-0.5] <- mat[mat<0.5&mat>-0.5]/10
...
mat <- mat2
mat[p.mat2>=0.05] <- 0
mat[mat<0.5&mat>-0.5] <- mat[mat<0.5&mat>-0.5]/3
network <- graph_from_adjacency_matrix(mat, weighted=T, mode="undirected", diag=F)
...
V(network)$x <- l[, 1]
V(network)$y <- l[, 2]
V(network)$color1 <- colors1
V(network)$color2 <- colors2
V(network)$radius <- log10(node.size)/5
...
df2 <- df2 %>% mutate(distance_to_center = runif(nrow(df2), 0, 1)*radius*2.2,
...
ggraph(network,layout=l) +
  geom_edge_density(aes(fill = edge.color)) + 
  scale_edge_fill_identity() +
  geom_edge_link0(alpha = abs((E(network)$weight)*2),
  ...
  scale_edge_color_gradient2(name = "Link Strength", 
                             ...
                             midpoint = 0, 
                             limit = c(-1,1)) +
  geom_point(data = df2,aes(x = x, y = y), size = 0.5,alpha=0.3,
             col = df2$name) +
