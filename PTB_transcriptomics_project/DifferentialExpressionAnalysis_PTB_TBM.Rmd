...
mypie <- function(x, y, values, radius, edges=200, col=NULL, angle=45,width=NULL,
                  density=NULL, border=NULL, lty=NULL, init.angle=90, ...) {
  values <- c(0, cumsum(values)/sum(values))
  dx <- diff(values)
  nx <- length(dx)
  ...
  col <- rep(col, length.out = nx)
  border <- rep(border, length.out = nx)
  border_width <- rep(width, length.out = nx)
  lty <- rep(lty, length.out = nx)
  angle <- rep(angle, length.out = nx)
  density <- rep(density, length.out = nx)
  t2xy <- function(t) {
  ...
  for (i in 1:nx) {
    ...
    P <- t2xy(seq.int(values[i], values[i + 1], length.out = n))
    polygon(x+c(P$x, 0), y+c(P$y, 0), density = density[i], angle = angle[i],
            border = border[i],lwd = border_width[i], col = col[i], lty = lty[i], ...)
  }
}


shapes[['pie']]$plot <- function (coords, v = NULL, params) 
{
    getparam <- function(pname) {
        p <- params("vertex", pname)
        if (length(p) != 1 && !is.null(v)) {
    ...
    for (i in seq_len(nrow(coords))) {
        ...
        mypie(x = coords[i, 1], y = coords[i, 2], pie, radius = vertex.size[i], 
            edges = 200, col = col, angle = na.omit(vertex.pie.angle[c(i, 1)])[1], 
            density = na.omit(vertex.pie.density[c(i, 1)])[1], 
            border = na.omit(vertex.frame.color[c(i, 1)])[1], 
            lty = na.omit(vertex.pie.lty[c(i, 1)])[1],
            width = vertex.frame.width)
      }
    }
...
plot(g, vertex.shape="pie", vertex.pie=pie, vertex.frame.color="black", vertex.frame.width=2,
