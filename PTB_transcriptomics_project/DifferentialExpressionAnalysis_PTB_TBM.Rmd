...
module_all <- sort(c(module_discovery,module_validation))
module_all
...
colors <- data.frame(color = colors,#natparks.pals("Arches",n=18),
                      levels = sort(c(module_discovery,module_validation)))
...
#dev.off()
#
png(file=paste0(path_output,"/Figure_tmp.png"),res=900, units = 'in', width=6.5, height = 6.5)
...
mat <- mat2
mat[p.mat2>=0.05] <- 0
mat[mat<0.5&mat>-0.5] <- mat[mat<0.5&mat>-0.5]/1.5
network <- graph_from_adjacency_matrix(mat, weighted=T, mode="undirected", diag=F)
edge.color <- factor(c("#FF681E", "#001164")[(E(network)$weight<0)+1])

png(file=paste0(path_output,"/Figure_tmp.png"),res=900, units = 'in', width=10, height = 7)
ggraph(network) +
  geom_edge_density(aes(fill = edge.color),n=20) +
  ...
  low = "#001164",mid='white',high = "#FF681E",midpoint = 0,limit = c(-1,1)) +
  geom_node_point(fill =node.colors, colour = 'black' ,size=10,stroke=1,shape = 21)+
...
+
  geom_edge_density(aes(fill = edge.color),col = )+
  scale_edge_colour_gradient(low = "#001164",
                             high = "#FF681E")+
  scale_color_manual(values = node.colors, breaks = node.colors) +
  scale_fill_manual(values = node.colors, breaks = node.colors) + theme_void()
