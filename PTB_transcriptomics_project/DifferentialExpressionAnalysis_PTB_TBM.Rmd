...
plot(network, 
     vertex.label=NA, 
     vertex.size=log2(node.size)*3.5/1.1,
     vertex.color=node.colors, 
     edge.color=NA,
     add=TRUE) 
...
```{r} 
library(graphlayouts) hubgenes_CT \<- read.csv(paste0(path_input,"Data/Modules-Hubgenes_CT_value.csv"))$gene brown_module <- data.frame(genes=colnames(multiExpr[[1]]$data),module = moduleColors) %\>% filter(module=='brown') brown_genes \<- brown_module\$genes adj \<- TOM1 adj \<- adj[brown_genes,brown_genes] diag(adj) \<- NA adj[adj \< quantile(adj, probs = 0.3, na.rm = T)] = 0 network \<- graph_from_adjacency_matrix(adj, weighted=T, mode="undirected", diag=F)

e \<- get.edgelist(network,names=FALSE) l \<- qgraph.layout.fruchtermanreingold(e, vcount=vcount(network), area=12\*(vcount(network)\^2), repulse.rad=(vcount(network)\^3.1) )

l \<- qgraph.layout.fruchtermanreingold(e, vcount=vcount(network), area=120\*(vcount(network)\^2), repulse.rad=(vcount(network)\^8)) l \<- qgraph.layout.fruchtermanreingold(e,vcount=vcount(network)) png(file=paste0(path_output,"/Figure_brown_tmp.png"),res=900, units = 'in', width=6.5, height = 6.5)

plot(network, layout= l,#l, vertex.label='', vertex.size = ifelse(V(network)$name%in%hubgenes_CT, 25,2),  vertex.color = ifelse(V(network)$name%in%hubgenes_CT,'purple','darkgreen'), edge.width=(E(network)\$weight)) dev.off()
