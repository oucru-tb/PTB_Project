...
```{r}
highlow_mat <- ICvslowCNIH4_mat
highlow_mat[upper.tri(ICvslowCNIH4_mat,diag=F)] <- ICvshighCNIH4_mat[upper.tri(ICvshighCNIH4_mat,diag=F)]
p.highlow_mat <- p.ICvslowCNIH4_mat
p.highlow_mat[upper.tri(p.ICvslowCNIH4_mat,diag=F)] <- p.ICvshighCNIH4_mat[upper.tri(p.ICvshighCNIH4_mat,diag=F)]

p12<-ggcorrplot(highlow_mat,
           p.mat = p.highlow_mat, 
           colors = col1,
           lab = F,
           lab_size = 2,
  ...
  labs(fill="Corr")+
  theme(axis.text.x = element_text(face="bold", colour="black", size=12,angle=135),
        axis.text.y = element_text(face="bold", colour="black", size=12,angle=135),
        legend.text = element_text(size = 15),
        legend.title = element_text( colour="black", size=12))+
...
print(p12, vp = viewport(angle = -135))

png(file=paste0(path_output,"/Figure4C_differential_coexpression_heatmap3.png"),res=600, units = 'in', width=20, height =10)
...
data_long <- merge(impute_df,lowCNIH4vsIC_df,by=0)
data_long <- gather(data_long, condition, measurement, B.cells.naive:Neutrophils, factor_key=TRUE)
...
data_long <- merge(impute_df,highCNIH4vsIC_df,by=0)
data_long <- gather(data_long, condition, measurement, B.cells.naive:Neutrophils, factor_key=TRUE)
...
tmp0 <- read.csv(paste0(path_input,'Raw_data/LM22_ref_CNIH4/CIBERSORTxHiRes_NA_Neutrophils_Window20.txt'),sep='\t')
rownames(tmp0) <- tmp0$GeneSymbol
tmp0$GeneSymbol <- NULL
tmp0 <- t(tmp0) %>% as.data.frame() %>% dplyr::rename(Neutrophils = CNIH4)
...
p <- ggplot() +
  # Top
  geom_density(data=highCNIH4_neutrophils, 
               aes(x = Neutrophils, y = ..density..), fill="#69b3a2" ) +
  geom_label( aes(x=4.5, y=0.25, label="variable1"), color="#69b3a2") +
  # Bottom
  geom_density(data=lowCNIH4_neutrophils,
               aes(x = Neutrophils, y = -..density..), fill= "#404080") +
...
CNIH4lv_neutrophils <- merge(vst_data2,tmp0,by=0)[,c('Neutrophils','level')]
...
p<-ggplot(data=CNIH4lv_neutrophils,
       aes(x=reorder(level, Neutrophils,na.rm = TRUE),
           y=Neutrophils)) +
  theme_bw() +
  geom_violin(aes(color=level,fill=level),
              width=0.5,alpha=0.2,
              trim=FALSE)+
  scale_fill_manual(values=c("#BF616A","#81A1C1"))+
  scale_color_manual(values=c("#BF616A","#81A1C1"))+
  geom_boxplot(aes(color=level),
               ...
               outlier.colour="red",
               outlier.shape=NA,outlier.size=3)+
    geom_jitter(aes(color=level,fill=level),
              width=0.05,
              height=0,
              size=2,
              alpha = 0.5)+
...
png(file=paste0(path_output,"/Figure_NeutrophilsInHighvsLowCNIH4.png"),res=900, units = 'in', width=8, height = 7)
p
dev.off()
```
