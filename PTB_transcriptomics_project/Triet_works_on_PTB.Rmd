...
png(file=paste0(path_output,"/Figure4B_scatterplot_CNIH4vsICs_all_PTB.png"),res=600, units = 'in', width=45, height =18)#30:18
ggpubr::ggarrange(
p[[1]],p[[2]],
p[[3]],p[[4]],
p[[5]],p[[6]],
...
IC_names<-c('CD200', 'CD58','TNFSF4', 'CD200R1', 'IDO1','TNFRSF18', 'TNFSF18',
             'CD28','BTLA', 'CD27', 'HAVCR2', 'CD274', 'TNFSF14', 'TNFRSF4',
             'CD276', 'TNFRSF14', 'PDCD1', 'CD86', 'TNFSF9', 'TIGIT','CD80',
             'ICOS', 'TNFRSF9', 'CTLA4', 'LAG3','LGALS9','CNIH4',
...
# CNIH4 vs 26 ICs
n = 27
mat1 <- ICvsCNIH4_mat[1:n,1:n]
p.mat1 <- p.ICvsCNIH4_mat[1:n,1:n]
labels1 <-round(mat1,2)
vals1 <- labels1
vals1[p.mat1 > 0.05] <- 0
labels1[p.mat1 > 0.05] <- ''
...
# CNIH4 vs other
mat2 <- ICvsCNIH4_mat[28:dim(ICvsCNIH4_mat)[1],1:27]
mat2 <- mat2[,col.order]
p.mat2 <- p.ICvsCNIH4_mat[28:dim(ICvsCNIH4_mat)[1],1:27]
...
p1<-ggplot(data=df, aes_string(x='CNIH4',y=as.name('CD274/PDL1')))+
  geom_point(alpha=0.2,col='red')+
  theme_bw()+
  stat_poly_line(se=T) +
  stat_cor(cor.coef.name = "r",size = 7,method = 'spearman')+
  theme_bw()+ 
  ylab(paste0(as.name('CD274/PDL1')," expression"))+
  xlab("CNIH4 expression")+
  theme(plot.title = element_text(size = 15),
        legend.position = "none",
        axis.text = element_text(size = 15),
        ...
        legend.key = element_rect(colour = NA),
        strip.text=element_text(face = "bold",size=12))
png(file=paste0(path_output,"/Figure4B_scatterplot_CNIH4vsPDL1_TBM.png"),res=600, units = 'in', width=15/2, height =9/2)
p1
dev.off()
```
...
png(file=paste0(path_output,"/Figure4B_scatterplot_CNIH4vsICs_all_TBM.png"),res=600, units = 'in', width=45, height =18)
ggpubr::ggarrange(
p[[1]],p[[2]],
p[[3]],p[[4]],
p[[5]],p[[6]],
...
dim(tbm_vst2) # 295 31427
```

```{r}
lowCNIH4vsIC_df <- tbm_vst2 %>% select(append(IC_names[1:27],'level')) %>% filter(level=='LOW') %>% select(-level)
highCNIH4vsIC_df <- tbm_vst2 %>% select(append(IC_names[1:27],'level')) %>% filter(level=='HIGH') %>% select(-level)
```
...
emat <- t(tbm_vst2%>%select(IC_names[1:27])) #tranpose, 23+1
condition <- tbm_vst2$level # high and low
scores <- dcScore(emat,
                  condition,
                  dc.method = "zscore",
                  cor.method = 'spearman')
...
emat <- t(tbm_vst2%>%select(IC_names[1:26])) #tranpose
condition <- tbm_vst2$level # high and low
scores <- dcScore(emat, 
                  condition,
                  dc.method = "zscore",
...
# png(file=paste0(path_output,"/Figure4C_differential_coexpression_boxplot_TBM.png"),res=900, units = 'in', width=8, height = 7)
# p
# dev.off()
```
