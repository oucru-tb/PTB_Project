...
# clinical <- read_csv(paste0(path_input,"/Raw_data/datafull_08_08_2023.csv"))%>% as.data.frame()
# View(clinical)
...
#  
# mri26TB <- read_csv(paste0(path_input,"/Raw_data/check_list_26.csv"))%>% as.data.frame() %>% drop_na(date_take)
# mri27TB <- read_csv(paste0(path_input,"/Raw_data/check_list_27.csv"))%>% as.data.frame() %>% drop_na(date_take)
# load(paste0(path_input,'/Raw_data/Baseline_26TB_27TB_18August2023_27TB_entry1_Khanh.RData'))
# 
# dim(mri26TB %>% drop_na(date_take))
# 
# cinical_cols <- c('study','SEX','AGE','WEIGHT','GCS','TBMGRADE','CSFLYMLE','CSFGLUC','LACTATE','study','ULTIMATE_diagnosis')
# clinical_mri <- Baseline_26TB_27TB %>% filter(USUBJID %in% c(unique(mri26TB$PatientID),unique(mri27TB$PatientID)))%>%select(all_of(cinical_cols))
# 
# table <- clinical_mri %>% 
#   tbl_summary(
...
#                  WEIGHT~"Weight (kg)",
#                  TBMGRADE~"TBM Grade",
#                  CSFLYMLE~"CSF lymphocyte count (cell/uL)",
#                  CSFGLUC~"CSF glucose",
#                  LACTATE~"CSF lactate",
#                  ULTIMATE_diagnosis~'TBM diagnosis'
#                  ),
#     
#   ) %>%add_overall() %>%
...
#   modify_footnote(
#   all_stat_cols() ~ "Median (Quartile 1, Quartile 3)")%>%
#   bold_labels()
# table%>%as_flex_table() %>% save_as_docx(path =paste0(path_output,"/MRI.docx"))
# table
```
...
p <- ggraph(network,layout=l) +
  ...
  geom_point(data = df2,aes(x = x, y = y), size = 0.5,alpha=0.9,
             col = df2$colors) +
...
model1 <- lm(CD58 ~ CD274 + CNIH4, data=df,
             #family="binomial"
             )
...
```{r}
smtx_counts <- read.csv(paste0(path_input,"/Data/salmon-tx-counts.tsv"),sep='\t')
row.names(smtx_counts) <- smtx_counts$transcript_name
smtx_counts <- t(smtx_counts[,7:302])
```
