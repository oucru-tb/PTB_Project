...
#                             "Macrophages.M1"
#                             )))  %>% 
#   dplyr::rename('HAVCR2/TIM-3'='HAVCR2',
#          'CD274/PDL1'='CD274',
#          'Lymphocytes'='LYMLE',
#          'LGALS9/Galactin-9'='LGALS9',
#          'PDCD1/PD1'='PDCD1'
...
p<-ggplot(data=data_long,aes(x=condition,y=measurement))+
...
tmp1<-merge(vst_data,tmp[,2, drop=FALSE],by=0) #%>% select(append(IC_names[1:26],c('LIMS_ID','select')))
...
ICvslowCNIH4_df <- lowCNIH4_df %>% select(IC_names[1:26])
ICvshighCNIH4_df <- highCNIH4_df %>% select(IC_names[1:26])
###############################################################
cor_method = 'spearman'
...
```{r}
# mat1 <- ICvslowCNIH4_mat
# p.mat1 <- p.ICvslowCNIH4_mat
# labels1 <-round(mat1,2)
# vals1 <- labels1
# vals1[p.mat1 > 0.05] <- 0
# labels1[p.mat1 > 0.05] <- ''
...
#                show_colnames = T,legend = F)
# 
# mat1 <- ICvshighCNIH4_mat
# p.mat1 <- p.ICvshighCNIH4_mat
# labels1 <-round(mat1,2)
# vals1 <- labels1
# vals1[p.mat1 > 0.05] <- 0
# labels1[p.mat1 > 0.05] <- ''
...
list <- get.edgelist(dcnet)
E(dcnet)$color <-  apply(as.data.frame(get.edgelist(dcnet)), 2, 
                 FUN = function(x) V(dcnet)$color[x[1]])
...
g<-plot(dcnet, 
     ...
     width=2,
     #edge.color= E(dcnet)$color, 
     layout= layout_nicely
     )

plot(dcnet,layout=layout_with_fr,vertex.size=4,vertex.label=NA)
...
ggplot(data=df,
  ...
  #geom_jitter(aes(color=CNIH4level),height=0,width=0.2,alpha=0.3)+
  theme_bw()+
  theme(axis.text.x = element_text(face="bold", colour="black", size=12),
        axis.text.y = element_text(face="bold", colour="black", size=12),
        legend.text = element_text(size = 15),
...
ggplot(data=df,
  ...
  theme(axis.text.x = element_text(face="bold", colour="black", size=12),
        ...
        axis.title.y=element_text(size=14,face="bold"))+
  xlab('') + 
  ylab('Absolute Coexpression of 25 immune checkpoint genes')
```
