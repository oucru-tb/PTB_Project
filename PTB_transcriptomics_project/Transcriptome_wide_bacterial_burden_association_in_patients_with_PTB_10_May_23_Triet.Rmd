...
tmp<-filter(All_Fluidigm_data,(Study%in% c('28TB','29TB'))&(Sample_Type=='Blood')&(Timepoint=='D0')&(Ct_Call=='Pass')&(EvaGreen_Name%in%common_list))
...
22 immune checkpoint gene vs CNIH4

```{r}

tmp <- merge(ICGvsCNIH4_df, impute_df, by='row.names', all=TRUE)
tmp <- na.omit(tmp)
```
...
library(tidyverse)
library(stats)
ICG_names<-c('CD200', 'TNFSF4', 'CD200R1', 'IDO1','TNFRSF18', 'TNFSF18',
...
ICGvsCNIH4_df <- vst_data %>% select(ICG_names)

cor_method = 'spearman'
ICGvsCNIH4_mat <- cor(ICGvsCNIH4_df,
...
p.ICGvsCNIH4_mat <- cor.pval.test(ICGvsCNIH4_df, 
                                  ICGvsCNIH4_df, 
                                  method=cor_method, 
                                  use='pairwise.complete.obs')
...
p2 <- ggcorrplot(ICGvsCNIH4_mat,
                 p.mat = p.ICGvsCNIH4_mat, 
                 hc.order = TRUE,
                 type = 'full',
                 color=col,
                 outline.color = 'black',
...
```{r}
library(pheatmap)
myColor <- colorRampPalette(c("#001164", 'white', "#FF681E"))(25)
