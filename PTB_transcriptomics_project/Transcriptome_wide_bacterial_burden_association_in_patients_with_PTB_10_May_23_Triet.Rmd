...
ICvsCNIH4_df <- merge(vst_data, impute_df, by=0) %>% tibble::column_to_rownames('Row.names') %>%
...
healthy_df <- count_data_adjusted_ComBat %>% filter(LIMS_ID%in%healthycontrol_id$LIMS_ID)
qt<-quantile(healthy_df$CNIH4,c(0.025,0.975))
qt
tmp<- vst_data %>% select(CNIH4)
tmp %<>% mutate(select=ifelse((CNIH4<qt[1]),"LOW",ifelse((CNIH4>qt[2]),'HIGH','_')))
table(tmp$select)
