...
ICvsCNIH4_df <- merge(ICvsCNIH4_df, impute_df, by=0) %>% 
  select(append(IC_names,c("Neutrophils",
                            "Monocytes",
                            "T.cells.CD8",
                            "T.cells.CD4.naive",
...
for(i in 1:length(names(ICvsCNIH4_df))){
  if(names(ICvsCNIH4_df)[i]=='CNIH4'){
    print(i)
    mylist <- append(mylist, 'red')
  }
  else{
    mylist <- append(mylist, 'black')
  }
}
mylist[28]<-'black'
mylist[30]<-'red'
...
p2 <- ggcorrplot(ICvsCNIH4_mat,
      ...
      theme(#axis.text.y = element_text(face="bold", colour="black", size=12),
            #axis.text.x = element_text(face="bold", colour="black", size=12),
            axis.text.y = element_markdown(face="bold", colour=mylist, size=12),
            axis.text.x = element_markdown(face="bold", colour=mylist, size=12),
            legend.text = element_text(size = 15),
            legend.title = element_text( colour="black", size=12))+
      theme(legend.position="right")


png(file=paste0(path_output,"/Figure_correlation_CNIH4vsICs.png"),res=600, units = 'in', width = 6.5, height =6.5)
p2
dev.off()
# grid.newpage()
```
...
pheatmap(ICGvsCNIH4_mat,  color = myColor, 
