...
# load(file="/home/ubuntu/tb_volume/PTB_transcriptomics_project/Analysis/Data/signal_genes_discovery.Rdata")
save(signal_genes,R_sq,file="Data/signal_genes_discovery.Rdata")
```
...
# load(file="Data/signal_genes_validation.Rdata")
save(signal_genes,R_sq,file="Data/signal_genes_validation.Rdata")
```
...
file_names<-paste0("Data/correlation_set_1_set_",gsub("-","_", Sys.Date()),".Rdata")
...
load(file="Data/correlation_set_1_set_2023_07_16.Rdata")
...
save(R2_sum_set_1_set,file="Data/R2_sum_set_1.Rdata")
```
...
file_names<-paste0("Data/correlation_set_2_set_",gsub("-","_", Sys.Date()),".Rdata")
save(branks,R2,file=file_names)
```
...
```{r fig.height=10, fig.width=10, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
load(file="Data/correlation_set_2_set_2023_07_16.Rdata")
gene_names<-colnames(branks)
branks<-as.matrix(branks)
branks_sum<-matrixStats::colQuantiles(branks,probs = c(0.025,0.975,0.5)) %>% as.data.frame()
branks_sum$means_ranks<-colMeans(branks)
...
save(R2_sum_set_2_set,file="Data/R2_sum_set_2.Rdata")
```

### Combine signals from the two independent sets

```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
load(file="Data/R2_sum_set_1.Rdata")
load(file="Data/R2_sum_set_2.Rdata")
...
write.table(a,file="Data/top_hit.txt",quote = F,row.names = F)
...
save(R2_top_hit,file="Data/R2_top_hit.Rdata")
save(common_list,file="Data/common_list.Rdata")
```
...
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
load(file="Data/R2_top_hit.Rdata")
```
...
png('Results/Figures_Tables/Ranking_R2_signal_genes.png',res=400, units = 'in', width = 20, height =20)
...
png('Results/Figures_Tables/correlation_top_hit.png',res=400, units = 'in', width = 10, height =5)
cowplot::plot_grid(p1,p2, nrow = 1)
dev.off()
```
...
## The imporatnt genes consist of the high ranked genes with the hub-gene from the WGCNA analysis
load(file="Data/Upregulation_Module_membership_bacterial_burden_profile.Rdata")
load(file="Data/common_list.Rdata")
important_genes<-unique(c(common_list,output_hubs$gene))
save(important_genes,file="Data/Important_gene.Rdata")
dat_top_hit<-clinical_RNA[c(important_genes,"log2_CT")]
...
## assume unimodality:
#stabsel_parameters(mod, q=3, PFER = 1,sampling.type = "SS",B=5000)
set.seed(12345)
(stab.boot <- stabs::stabsel(mod, q =3, PFER = 1, mc.cores = 12,sampling.type = "SS",assumption = "r-concave",B=500))
...
dat_plot %<>% mutate(variable=gsub("bbs\\(","",variable))
dat_plot %<>% mutate(variable=gsub("\\, df = dfbase\\)","",variable))
dat_plot %>%
  mutate(name = reorder(variable, Selected.probability)) %>%
  ggplot(aes(y=name,x=Selected.probability))+
  geom_segment(aes(x = 0,
                   xend = Selected.probability,
                   y = name,yend=name),
               color = "gray", lwd = 1)+geom_point(size = 4, pch = 21, bg = 4, col = 1)+ #+geom_point(col="#2c7fb8",size=4,shape=15) +
...
png(file=paste0(path_output_2,"/Figure_2C_GBX.png"),res=450, units = 'in', width =7, height =10)
p1
