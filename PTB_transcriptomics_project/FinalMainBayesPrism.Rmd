...
# sc.stat <- plot.scRNA.outlier(input=sc.dat,cell.type.labels=celltypes, species="hs", return.raw=TRUE)
...
plot_regs <- function(theta1,gt1,name='data') {
  ...
  regs <- annotate_figure(regs, top = text_grob(paste0("Bulk PTB, Reference SC ",name, "\npval.max=", pval, ", lfc.min=",lfc), 
...
InstaPrism.res.initial = InstaPrism(input_type = 'refPhi',bulk_Expr = t(bk.dat) ,refPhi = refPhi_obj, n.iter=100,
                                    convergence.plot=T, verbose = T)
...
```{r}
#theta <- as.data.frame(t(InstaPrism.res.initial@Post.ini.ct@theta))
...
regs <- plot_regs(theta1,gt1,name='Azimuth #3 (Updated)')
regs
```
