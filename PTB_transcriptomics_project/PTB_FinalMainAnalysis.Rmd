...
celltypes <- colnames(ptb.theta)[18:20]
celltypes <- colnames(ptb.theta)[1:17]
...
mypal <- pal_jco("default", alpha = 0.6)(16)
show_col(mypal)
```
...
all.z0 <- round(t(as.matrix(all.z)))

group = rep(c('Healthy','PTB','TBM27','TBM26'),times=c(30,296,207,74))
group = as.factor(rep(group,times=3))
celltype = as.factor(rep(level2,times=c(607,607,607)))
coldata <- data.frame(group, celltype)

dds = DESeqDataSetFromMatrix(countData = all.z0,
                              colData = coldata,
                              design = ~ group + celltype)
all.z_vst <- vst(dds, blind=FALSE)
...
col_fun = colorRamp2(c(6,8,9), col)
